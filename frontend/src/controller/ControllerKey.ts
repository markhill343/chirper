class ControllerKey {
  private v_key_events: Array<string>;
  private f_callback: EventListener; // KeyboardEventListener doesn't esits ...
  ...
  constructor(
    ...
    this.v_key_events =
      typeof p_key_events === "string" ? [p_key_events] : p_key_events;

    const c_keys = typeof p_keys === "string" ? [p_keys] : p_keys,
      c_targets = typeof p_targets === "string" ? [p_targets] : p_targets;
    this.f_callback = (
      p_event: Event // (p_event: KeyboardEvent) =>
    ) => {
      const c_key = (p_event as KeyboardEvent).key,
        c_target = (p_event.target as HTMLInputElement).id;

      if (
        c_keys.includes(c_key) &&
        (!c_targets || c_targets.includes(c_target))
      ) {
