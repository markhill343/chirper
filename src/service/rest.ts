...
const f_options = (
  ...
  f_json = async (
    ...
    const res = await fetch(p_url, f_options(p_method, p_token, p_data)),
      headers: TStringRecord = {};

    res.headers.forEach(
      (value: string, name: string) => (headers[name] = value)
    );
...
async function postJson(
  p_token: string | null,
  p_url: string,
  p_data: Record<string, TJsonAtom> = {}
) {
...
export default { getJson, postJson, putJson, patchJson, deleteJson };
