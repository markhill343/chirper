...
accounts
  ...
  .get(isAuthorized, refreshToken, async (req, res) => {
    const { status, data } = await accountsDB.getAccounts(
      req.isAdmin ? req.query.search : req.id
    );
...
accounts
  ...
  .get(isAuthorized, refreshToken, async (req, res) => {
    ...
    const { status, data } = await accountsDB.getAccount(req.params.id);
  ...
  .put(
    ...
    async (req, res) => {
      ...
      const { result } = await accountsDB.putAccount(req.params.id, req.body);
  ...
  .delete(isAuthorized, refreshToken, async (req, res) => {
    if (!req.isAdmin && req.id !== req.params.id) {
      res.status(401).json({ message: "not authorized" });
    }
